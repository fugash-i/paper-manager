...
DOIやPDFからメタデータを取得し、翻訳付きでNotionに追加します。また、Notion上のデータをNature形式やBibTeX形式でエクスポートすることも可能です。
...
- **エクスポート**: Notionに保存した論文を「Nature形式」や「BibTeX」で出力（引用キー検索対応）。
...
### GUIで使う (推奨)
`paper-manager.command` ファイルをダブルクリックしてください。
